...
import { OpportunitySplitView } from './HuntingSplitView';
import { OpportunitiesTable } from '../../backoffice/OpportunitiesTable';
import { useAuthStore } from '../../../store/useAuthStore';
import { toast } from 'sonner';
import { fetchApi } from '../../../services/api.client';
import { NewPredioModal } from '../../../components/opportunities/NewPredioModal';
...
export const KanbanBoard: React.FC = () => {
  ...
  const fetchCards = async () => {
    ...
    try {
      const { huntingService } = await import('../../../services/hunting.service');
      const data = await huntingService.getAll();
  ...
  const handleCompanyChange = async (cardId: string, newCompanyId: string) => {
    try {
      const { huntingService } = await import('../../../services/hunting.service');
      await huntingService.updateForms(cardId, { companyId: newCompanyId });
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      const { huntingService } = await import('../../../services/hunting.service');
      await huntingService.transitionStage(String(active.id), targetStageObj.id);
  ...
  const confirmClosing = async () => {
    ...
    try {
      const { huntingService } = await import('../../../services/hunting.service');
      await huntingService.transitionStage(String(currentCard.id), targetStageObj.id, lostReason);
          ...
          onApprove={async (towersData) => {
            ...
            if (targetStage !== currentStage && activePipeline) {
              ...
              try {
                const { huntingService } = await import('../../../services/hunting.service');
                const targetStageObj = activePipeline.stages[targetStage];
                await huntingService.transitionStage(
