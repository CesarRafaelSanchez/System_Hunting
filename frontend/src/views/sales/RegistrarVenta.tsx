...
export const RegistrarVenta: React.FC = () => {
  ...
  const fetchVentas = async () => {
    ...
    try {
      ...
      // Filtrar por creador o responsable (Asesor de Ventas)
      const filtered = (user?.role === 'ASESOR_VENTAS' || user?.role === 'HUNTER') 
  ...
  const filteredVentas = ventas.filter((p) => {
    const direccion = (p.subtitle || '').toLowerCase();
    const razonSocial = (p.title || '').toLowerCase();
    const q = searchTerm.toLowerCase();
    return direccion.includes(q) || razonSocial.includes(q);
  });
      ...
      {showForm ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
              <h2 className="text-xl font-bold text-gray-800">Mis Ventas Registradas</h2>
              <p className="text-sm text-gray-500">Historial de Ventas Fijas B2B</p>
            </div>
            <button 
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                  <th className="p-4 border-b border-gray-200 font-semibold rounded-tl-lg">Razón Social</th>
                  <th className="p-4 border-b border-gray-200 font-semibold">Dirección de Instalación</th>
                  <th className="p-4 border-b border-gray-200 font-semibold">Tecnología / Play</th>
                  <th className="p-4 border-b border-gray-200 font-semibold text-center">Cargo Fijo</th>
                  <th className="p-4 border-b border-gray-200 font-semibold rounded-tr-lg">Estado</th>
                </tr>
              </thead>
                    ...
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-gray-800">{p.title || 'Sin nombre'}</p>
                            <p className="text-xs text-gray-500 uppercase">{p.company?.name || 'GRUPO FS'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-4">
